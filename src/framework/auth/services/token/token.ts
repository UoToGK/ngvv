...
export abstract class DyAuthToken {
  ...
  getName(): string {
    return (this.constructor as DyAuthTokenClass).NAME;
  }
...
export class DyAuthTokenNotFoundError extends Error {
...
export class DyAuthIllegalTokenError extends Error {
  constructor(message: string) {
    super(message);
    Object.setPrototypeOf(this, new.target.prototype);
  }
...
export class DyAuthEmptyTokenError extends DyAuthIllegalTokenError {
  constructor(message: string) {
    super(message);
    Object.setPrototypeOf(this, new.target.prototype);
  }
...
export class DyAuthIllegalJWTTokenError extends DyAuthIllegalTokenError {
  constructor(message: string) {
    super(message);
    Object.setPrototypeOf(this, new.target.prototype);
  }
...
export interface DyAuthRefreshableToken {
  getRefreshToken(): string;
  setRefreshToken(refreshToken: string);
}

export interface DyAuthTokenClass<T = DyAuthToken> {
  NAME: string;
  new (raw: any, strategyName: string, expDate?: Date): T;
}

export function dyAuthCreateToken<T extends DyAuthToken>(tokenClass: DyAuthTokenClass<T>,
                                  token: any,
...
export function decodeJwtPayload(payload: string): any {
  ...
  if (payload.length === 0) {
    throw new DyAuthEmptyTokenError('Cannot extract from an empty payload.');
  }

  const parts = payload.split('.');

  if (parts.length !== 3) {
    throw new DyAuthIllegalJWTTokenError(
 ...
 */
export class DyAuthSimpleToken extends DyAuthToken {

  static NAME = 'dy:auth:simple:token';
  ...
  constructor(protected readonly token: any,
    ...
    try {
      ...
      if (!(err instanceof DyAuthTokenNotFoundError)) {
 ...
 */
export class DyAuthJWTToken extends DyAuthSimpleToken {

  static NAME = 'dy:auth:jwt:token';
  ...
  protected parsePayload(): void {
    if (!this.token) {
      throw new DyAuthTokenNotFoundError('Token not found. ')
    }
 ...
 */
export class DyAuthOAuth2Token extends DyAuthSimpleToken {

  static NAME = 'dy:auth:oauth2:token';
  ...
  protected parsePayload(): void {
    if (!this.token) {
      ...
      if (!Object.keys(this.token).length) {
        throw new DyAuthEmptyTokenError('Cannot extract payload from an empty token.');
      }
    }
 ...
 */
export class DyAuthOAuth2JWTToken extends DyAuthOAuth2Token {

  static NAME = 'dy:auth:oauth2:jwt:token';
  ...
  protected parseAccessTokenPayload(): any {
    ...
    if (!accessToken) {
      throw new DyAuthTokenNotFoundError('access_token key not found.')
    }
