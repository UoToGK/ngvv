 ...
 *  ];
 *
 *  <dy-route-tabset [tabs]="tabs"></dy-route-tabset>
 * ```
 * ### Installation
 *
 * Import `DyRouteTabsetModule` to your feature module.
 * ```ts
 * @NgModule({
 *   imports: [
 *     // ...
      ...
      <ng-container *ngFor="let tab of tabs">
        <li *ngIf="tab.disabled; else enabled"
          ...
          <a tabindex="-1" class="tab-link">
            <dy-icon *ngIf="tab.icon" [config]="tab.icon"></dy-icon>
        ...
        <ng-template #enabled>
          <li (click)="$event.preventDefault(); selectTab(tab)"
              ...
              class="route-tab">
            <a tabindex="-1" class="tab-link">
              <dy-icon *ngIf="tab.icon" [icon]="tab.icon"></dy-icon>
              <span *ngIf="tab.title" class="tab-text">{{ tab.title }}</span>
            </a>
          </li>
        </ng-template>
...
export class DyRouteTabsetComponent {
