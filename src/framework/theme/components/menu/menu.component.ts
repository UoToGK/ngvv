...
import { DyMenuInternalService, DyMenuItem, DyMenuBag, DyMenuService, DyMenuBadgeConfig } from './menu.service';
...
@Component({
  ...
  animations: [
    trigger('toggle', [
      state(DyToggleStates.Collapsed, style({ height: '0', margin: '0' })),
      state(DyToggleStates.Expanded, style({ height: '*' })),
      transition(`${DyToggleStates.Collapsed} <=> ${DyToggleStates.Expanded}`, animate(300)),
    ]),
  ],
})
export class DyMenuItemComponent implements DoCheck, AfterViewInit, OnDestroy {
  @Input() menuItem = <DyMenuItem>null;
  @Input() badge: DyMenuBadgeConfig;
  ...
  toggleState: DyToggleStates;

  constructor(protected menuService: DyMenuService,
              protected directionService: DyLayoutDirectionService) {}

  ngDoCheck() {
    this.toggleState = this.menuItem.expanded ? DyToggleStates.Expanded : DyToggleStates.Collapsed;
  }
  ...
  ngAfterViewInit() {
    this.menuService.onSubmenuToggle()
      .pipe(
        ...
        map(({ item }: DyMenuBag) => item.expanded),
        takeUntil(this.destroy$),
      )
      .subscribe(isExpanded => this.toggleState = isExpanded ? DyToggleStates.Expanded : DyToggleStates.Collapsed);
  }
  ...
  onToggleSubMenu(item: DyMenuItem) {
    this.toggleSubMenu.emit(item);
  }

  onHoverItem(item: DyMenuItem) {
    this.hoverItem.emit(item);
  }

  onSelectItem(item: DyMenuItem) {
    this.selectItem.emit(item);
  }

  onItemClick(item: DyMenuItem) {
 ...
 * ];
 * // ...
 * <dy-menu [items]="items"></dy-menu>
 * ```
 * ### Installation
 *
 * Import `DyMenuModule.forRoot()` to your app module.
 * ```ts
 * @NgModule({
 *   imports: [
 *     // ...
 ...
 * ```
 * and `DyMenuModule` to your feature module where the component should be shown:
 * ```ts
 * @NgModule({
 *   imports: [
 *     // ...
  ...
  template: `
    <ul class="menu-items">
      <ng-container *ngFor="let item of items">
        <li dyMenuItem *ngIf="!item.hidden"
            [menuItem]="item"
            [badge]="item.badge"
...
export class DyMenuComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  /**
   * List of menu items.
   * @type List<DyMenuItem> | List<any> | any
   */
  ...
  static ngAcceptInputType_autoCollapse: DyBooleanInput;

  protected destroy$ = new Subject<void>();

  constructor(@Inject(DY_WINDOW) protected window,
              @Inject(PLATFORM_ID) protected platformId,
              protected menuInternalService: DyMenuInternalService,
  ...
  ngOnInit() {
    ...
    this.menuInternalService
      .onAddItem()
      .pipe(
        filter((data: { tag: string; items: DyMenuItem[] }) => this.compareTag(data.tag)),
        takeUntil(this.destroy$),
      )
    ...
    this.menuInternalService
      .onGetSelectedItem()
      .pipe(
        filter((data: { tag: string; listener: BehaviorSubject<DyMenuBag> }) => this.compareTag(data.tag)),
        takeUntil(this.destroy$),
      )
      .subscribe((data: { tag: string; listener: BehaviorSubject<DyMenuBag> }) => {
  ...
  onAddItem(data: { tag: string; items: DyMenuItem[] }) {
  ...
  protected getHomeItem(items: DyMenuItem[]): DyMenuItem {
  ...
  protected getSelectedItem(items: DyMenuItem[]): DyMenuItem {
    let selected = null;
    items.forEach((item: DyMenuItem) => {
      if (item.selected) {
        selected = item;
      }
