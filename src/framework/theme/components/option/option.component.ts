...
import { DyFocusableOption } from '../cdk/a11y/focus-key-manager';
import { DyHighlightableOption } from '../cdk/a11y/descendant-key-manager';
import { DY_SELECT_INJECTION_TOKEN } from '../select/select-injection-tokens';
import { DySelectComponent } from '../select/select.component';
...
export class DyOptionComponent<T = any> implements OnDestroy, AfterViewInit, DyFocusableOption, DyHighlightableOption {
   ...
   * */
  @Output() selectionChange: EventEmitter<DyOptionComponent<T>> = new EventEmitter();

  /**
   * Fires when option clicked
   */
  protected click$: Subject<DyOptionComponent<T>> = new Subject<DyOptionComponent<T>>();
  get click(): Observable<DyOptionComponent<T>> {
    return this.click$.asObservable();
  }

  selected: boolean = false;
  protected parent: DySelectComponent;
   ...
   * */
  @HostBinding('attr.id')
  id: string = `dy-option-${lastOptionId++}`;

  constructor(@Optional() @Inject(DY_SELECT_INJECTION_TOKEN) parent,
  ...
  get multiple() {
    // We check parent existing because parent can be DySelectComponent or
    // DyAutocomplete and `miltiple` property exists only in DySelectComponent
    return this.parent ? (this.parent as DySelectComponent).multiple : false;
  }
