 ...
 **/
@Component({
  selector: 'dy-option-group',
  styleUrls: ['./option-group.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush,
  template: `
    <span class="option-group-title">{{ title }}</span>
    <ng-content select="dy-option, ng-container"></ng-content>
  `,
})
export class DyOptionGroupComponent implements AfterContentInit, OnDestroy {
  ...
  @ContentChildren(DyOptionComponent, { descendants: true }) options: QueryList<DyOptionComponent<any>>;
  ...
  protected updateOptionsDisabledState(): void {
    this.options.forEach((option: DyOptionComponent) => option.setDisabledByGroupState(this.disabled));
  }
