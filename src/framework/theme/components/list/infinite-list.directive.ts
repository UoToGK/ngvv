...
import { DyListItemComponent } from './list.component';

export class DyScrollableContainerDimentions {
  scrollTop: number;
  scrollHeight: number;
  clientHeight: number;
}
 ...
 *
 * ```html
 *  <dy-list dyInfiniteList [threshold]="500" (bottomThreshold)="loadNext()">
 *    <dy-list-item *ngFor="let item of items"></dy-list-item>
 *  </dy-list>
 * ```
 *
 ...
 * `dyListPageTracker` directive will help you to know, what page user currently viewing.
 ...
 */
@Directive({
  selector: '[dyInfiniteList]',
})
export class DyInfiniteListDirective implements AfterViewInit, OnDestroy {
  ...
  static ngAcceptInputType_listenWindowScroll: DyBooleanInput;
  ...
  @ContentChildren(DyListItemComponent) listItems: QueryList<DyListItemComponent>;

  constructor(
    private elementRef: ElementRef,
    private scrollService: DyLayoutScrollService,
    private dimensionsService: DyLayoutRulerService,
  ) {}
  ...
  checkPosition({ scrollHeight, scrollTop, clientHeight }: DyScrollableContainerDimentions) {
  ...
  private getContainerDimensions(): Observable<DyScrollableContainerDimentions> {
    if (this.elementScroll) {
