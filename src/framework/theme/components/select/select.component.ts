...
import { DyOverlayRef, DyPortalDirective, DyScrollStrategy } from '../cdk/overlay/mapping';
...
import { DyFocusKeyManager, DyFocusKeyManagerFactoryService } from '../cdk/a11y/focus-key-manager';
...
import { DY_SELECT_INJECTION_TOKEN } from './select-injection-tokens';
import { DyFormFieldControl, DyFormFieldControlConfig } from '../form-field/form-field-control';
import { DyFocusMonitor } from '../cdk/a11y/a11y.module';
import { DyScrollStrategies } from '../cdk/adapter/block-scroll-strategy-adapter';

export type DySelectCompareFunction<T = any> = (v1: any, v2: any) => boolean;
export type DySelectAppearance = 'outline' | 'filled' | 'hero';

@Component({
  selector: 'dy-select-label',
  template: '<ng-content></ng-content>',
})
export class DySelectLabelComponent {
}

export function dySelectFormFieldControlConfigFactory() {
  const config = new DyFormFieldControlConfig();
  config.supportsSuffix = false;
  return config;
}

/**
 * The `DySelectComponent` provides a capability to select one of the passed items.
 *
 * @stacked-example(Showcase, select/select-showcase.component)
 *
 * ### Installation
 *
 * Import `DySelectModule` to your feature module.
 * ```ts
 * @NgModule({
 *   imports: [
 *     // ...
 ...
 * In this case, `dy-select` will work only with arrays - accept arrays and propagate arrays.
 *
 ...
 *
 * ```html
 * <dy-select [(selected)]="selected"></dy-selected>
 * ```
 *
 ...
 *
 * Options in the select may be grouped using `dy-option-group` component.
 *
 ...
 *
 * Default `dy-select` size is `medium` and status is `basic`.
...
@Component({
  ...
  providers: [
    ...
    { provide: DY_SELECT_INJECTION_TOKEN, useExisting: DySelectComponent },
    { provide: DyFormFieldControl, useExisting: DySelectComponent },
    { provide: DyFormFieldControlConfig, useFactory: dySelectFormFieldControlConfigFactory },
  ],
})
export class DySelectComponent implements OnChanges, AfterViewInit, AfterContentInit, OnDestroy,
                                          ControlValueAccessor, DyFormFieldControl {
   ...
   */
  @Input() appearance: DySelectAppearance = 'outline';

  /**
   * Specifies class to be set on `dy-option`s container (`dy-option-list`)
   * */
  ...
  static ngAcceptInputType_hero: DyBooleanInput;
   ...
   */
  @Input()
  get compareWith(): DySelectCompareFunction {
    return this._compareWith;
  }
  set compareWith(fn: DySelectCompareFunction) {
  ...
  protected _compareWith: DySelectCompareFunction = (v1: any, v2: any) => v1 === v2;
  ...
  /**
   * List of `DyOptionComponent`'s components passed as content.
   * TODO maybe it would be better provide wrapper
   * */
  @ContentChildren(DyOptionComponent, { descendants: true }) options: QueryList<DyOptionComponent>;

  /**
   * Custom select label, will be rendered instead of default enumeration with coma.
   * */
  @ContentChild(DySelectLabelComponent) customLabel;

  /**
   * DyCard with options content.
   * */
  @ViewChild(DyPortalDirective) portal: DyPortalDirective;
   ...
   * */
  selectionModel: DyOptionComponent[] = [];
   ...
   */
  overlayPosition: DyPosition = '' as DyPosition;
  ...
  protected keyManager: DyFocusKeyManager<DyOptionComponent>;

  /**
   * If a user assigns value before content dy-options's rendered the value will be putted in this variable.
   ...
   * @docs-private
   **/
  status$ = new BehaviorSubject<DyComponentOrCustomStatus>(this.status);

  /*
   * @docs-private
   **/
  size$ = new BehaviorSubject<DyComponentSize>(this.size);
  ...
  constructor(@Inject(DY_DOCUMENT) protected document,
              ...
              protected focusKeyManagerFactoryService: DyFocusKeyManagerFactoryService<DyOptionComponent>,
  ...
  get selectionView() {
    if (this.selectionModel.length > 1) {
      return this.selectionModel.map((option: DyOptionComponent) => option.content).join(', ');
    }
  ...
  ngAfterContentInit() {
    this.options.changes
      .pipe(
        ...
        switchMap((options: QueryList<DyOptionComponent>) => from(Promise.resolve(options))),
  ...
  ngAfterViewInit() {
    ...
    this.zone.runOutsideAngular(() => setTimeout(() => {
      this.renderer.addClass(this.hostRef.nativeElement, 'dy-transition');
    }));
  }
   ...
   * */
  protected handleOptionClick(option: DyOptionComponent) {
    this.queue = null;
   ...
   * */
  protected reset() {
    this.selectionModel.forEach((option: DyOptionComponent) => option.deselect());
    this.selectionModel = [];
    this.hide();
   ...
   * */
  protected selectOption(option: DyOptionComponent) {
    if (this.multiple) {
   ...
   * */
  protected handleSingleSelect(option: DyOptionComponent) {
   ...
   * */
  protected handleMultipleSelect(option: DyOptionComponent) {
    if (option.selected) {
    ...
    this.emitSelected(this.selectionModel.map((opt: DyOptionComponent) => opt.value));
  }
  ...
  protected subscribeOnOptionClick() {
    ...
    this.options.changes
      .pipe(
        startWith(this.options),
        switchMap((options: QueryList<DyOptionComponent>) => {
          return merge(...options.map(option => option.click));
        }),
        takeUntil(this.destroy$),
      )
      .subscribe((clickedOption: DyOptionComponent) => this.handleOptionClick(clickedOption));
  }
  ...
  protected setSelection(value) {
    ...
    previouslySelectedOptions
      .filter((option: DyOptionComponent) => !this.selectionModel.includes(option))
      .forEach((option: DyOptionComponent) => option.deselect());
  ...
  protected selectValue(value) {
    const corresponding = this.options.find((option: DyOptionComponent) => this._compareWith(option.value, value));
