...
/**
 * The `DyToastComponent` is responsible for rendering each toast with appropriate styles.
 *
 * @styles
 *
 * toastr-border-style:
...
export class DyToastComponent implements OnInit {
  @Input()
  toast: DyToast;
  ...
  get hasIcon(): boolean {
    ...
    return !!(icon && (icon as DyIconConfig).icon);
  }
