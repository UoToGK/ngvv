...
import { DyFormFieldControl, DyFormControlState, DyFormFieldControlConfig } from './form-field-control';

export type DyFormControlAddon = 'prefix' | 'suffix';

function throwFormControlElementNotFound() {
  throw new Error(`DyFormFieldComponent must contain [dyInput]`)
}
...
export class DyFormFieldComponent implements AfterContentChecked, AfterContentInit, AfterViewInit, OnDestroy {

  protected readonly destroy$ = new Subject<void>();

  protected formControlState$ = new ReplaySubject<DyFormControlState>(1);
  ...
  @ContentChildren(DyPrefixDirective, { descendants: true }) prefix: QueryList<DyPrefixDirective>;
  @ContentChildren(DySuffixDirective, { descendants: true }) suffix: QueryList<DySuffixDirective>;

  @ContentChild(DyFormFieldControl, { static: false }) formControl: DyFormFieldControl;
  @ContentChild(DyFormFieldControlConfig, { static: false }) formControlConfig: DyFormFieldControlConfig;
  ...
  ngAfterViewInit() {
    ...
    this.zone.runOutsideAngular(() => setTimeout(() => {
      this.renderer.addClass(this.elementRef.nativeElement, 'dy-transition');
    }));
  }
  ...
  protected subscribeToFormControlStateChange() {
    ...
    combineLatest([disabled$, focused$, size$, status$, fullWidth$])
      .pipe(
        ...
        tap(({ size, fullWidth }) => {
          const formFieldClasses = [`dy-form-field-size-${size}`];
          if (!fullWidth) {
            formFieldClasses.push('dy-form-field-limited-width')
          }
  ...
  protected getAddonClasses(addon: DyFormControlAddon, state: DyFormControlState): string[] {
    const classes = [
      'dy-form-field-addon',
      `dy-form-field-${addon}-${state.size}`,
    ];

    if (state.disabled) {
      classes.push(`dy-form-field-addon-disabled`);
    } else if (state.focused) {
      classes.push(`dy-form-field-addon-${state.status}-highlight`);
    } else {
      classes.push(`dy-form-field-addon-${state.status}`);
    }

    return classes;
  }

  protected getFormControlConfig(): DyFormFieldControlConfig {
    return this.formControlConfig || new DyFormFieldControlConfig();
  }

  protected isStatesEqual(oldState: DyFormControlState, state: DyFormControlState): boolean {
