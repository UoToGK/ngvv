...
import { DyIconLibraries } from './icon-libraries';

export interface DyIconConfig {
  icon: string;
  pack?: string;
 ...
 * ```
 * This command will install Eva Icons pack. Then register `DyEvaIconsModule` into your app module:
 * ```ts
 * import { DyEvaIconsModule } from '@nebular/eva-icons';
 *
 * @NgModule({
 *   imports: [
 *     // ...
 ...
 * ```
 * Last thing, import `DyIconModule` to your feature module where you need to show an icon:
 * ```ts
 * import { DyIconModule } from '@nebular/theme';
 *
 * @NgModule({
 *   imports: [
 *     // ...
 ...
 * ```html
 * <dy-icon icon="star" status="warning"></dy-icon>
 * ```
 *
 * Colored icons:
 ...
 * ```html
 * <dy-icon icon="star" pack="font-awesome"></dy-icon>
 * ```
 * Additional icon settings (if available by the icon pack) could be passed using `options` input:
 *
 * ```html
 * <dy-icon icon="star" [options]="{ animation: { type: 'zoom' } }"></dy-icon>
 * ```
 *
 * @styles
 *
...
export class DyIconComponent implements DyIconConfig, OnChanges, OnInit {
   ...
   */
  @Input() status?: DyComponentOrCustomStatus;
   ...
   */
  @Input()
  get config(): string | DyIconConfig {
    return this._config;
  }
  set config(value: string | DyIconConfig) {
    if (!value) {
      return;
    }
  ...
  protected _config: string | DyIconConfig;

  constructor(
    protected sanitizer: DomSanitizer,
    protected iconLibrary: DyIconLibraries,
