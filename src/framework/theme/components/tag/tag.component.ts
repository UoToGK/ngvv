...
export type DyTagAppearance = 'filled' | 'outline';

export interface DyTagSelectionChange {
  tag: DyTagComponent;
  selected: boolean;
}
...
export class DyTagComponent implements AfterViewInit, OnDestroy, DyHighlightableOption {

  private _destroy$: Subject<DyTagComponent> = new Subject<DyTagComponent>();

  get destroy$(): Observable<DyTagComponent> {
  ...
  static ngAcceptInputType_selected: DyBooleanInput;
  ...
  static ngAcceptInputType_removable: DyBooleanInput;

  /**
   * Tag appearance: `filled`, `outline`.
   */
  @Input() appearance: DyTagAppearance = 'filled';
   ...
   */
  @Output() readonly remove: EventEmitter<DyTagComponent> = new EventEmitter<DyTagComponent>();

  @Output() readonly selectedChange: EventEmitter<DyTagSelectionChange> = new EventEmitter<DyTagSelectionChange>();

  @HostBinding('class.active')
  _isActive: boolean = false;

  @HostBinding('attr.id')
  _id: string = `dy-tag-${tagUniqueId++}`;
  ...
  ngAfterViewInit() {
    ...
    this.zone.runOutsideAngular(() => setTimeout(() => {
      this.renderer.addClass(this._hostElement.nativeElement, 'dy-transition');
    }));
  }
