 ...
 * ### Installation
 *
 * Import `DyInputModule` to your feature module.
 * ```ts
 * @NgModule({
 *   imports: [
 *     // ...
 ...
 *
 * `dyInput` could be applied to the following selectors - `input`, `textarea`:
 ...
 *
 * Use `<dy-form-field>` to add custom content to the input field.
 * First import `DyFormFieldModule`. Then put the input field and custom content into
 * `<dy-form-field>` and add `dyPrefix` or `dySuffix` directive to the custom content.
 * `dyPrefix` puts content before input and `dySuffix` after.
 *
 ...
 */
@Directive({
  selector: 'input[dyInput],textarea[dyInput]',
  providers: [
    { provide: DyFormFieldControl, useExisting: DyInputDirective },
  ],
})
export class DyInputDirective implements DoCheck, OnChanges, OnInit, AfterViewInit, OnDestroy, DyFormFieldControl {
   ...
   */
  @Input()
  fieldSize: DyComponentSize = 'medium';
   ...
   */
  @Input()
  shape: DyComponentShape = 'rectangle';
   ...
   * @docs-private
   **/
  size$ = new BehaviorSubject<DyComponentSize>(this.fieldSize);
