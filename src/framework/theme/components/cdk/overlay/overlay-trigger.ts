...
export type DyTriggerValues = 'noop' | 'click' | 'hover' | 'hint' | 'focus';
export enum DyTrigger {
  NOOP = 'noop',
  CLICK = 'click',
  HOVER = 'hover',
  HINT = 'hint',
 ...
 * */
export interface DyTriggerStrategy {
 ...
 * */
export abstract class DyTriggerStrategyBase implements DyTriggerStrategy {
 ...
 * */
export class DyClickTriggerStrategy extends DyTriggerStrategyBase {
 ...
 * */
export class DyHoverTriggerStrategy extends DyTriggerStrategyBase {
 ...
 * */
export class DyHintTriggerStrategy extends DyTriggerStrategyBase {
  show$: Observable<Event> = observableFromEvent<Event>(this.host, 'mouseenter')
    .pipe(
      // this `delay & takeUntil & repeat` operators combination is a synonym for `conditional debounce`
      // meaning that if one event occurs in some time after the initial one we won't react to it
 ...
 * */
export class DyFocusTriggerStrategy extends DyTriggerStrategyBase {
 ...
 * */
export class DyNoopTriggerStrategy extends DyTriggerStrategyBase {
  show$: Observable<Event> = EMPTY;
  hide$: Observable<Event> = EMPTY;
}

@Injectable()
export class DyTriggerStrategyBuilderService {

  protected _host: HTMLElement;
  protected _container: () => ComponentRef<any>;
  protected _trigger: DyTrigger;

  constructor(@Inject(DY_DOCUMENT) protected _document) {
  }

  trigger(trigger: DyTrigger): this {
  ...
  build(): DyTriggerStrategy {
    switch (this._trigger) {
      case DyTrigger.CLICK:
        return new DyClickTriggerStrategy(this._document, this._host, this._container);
      case DyTrigger.HINT:
        return new DyHintTriggerStrategy(this._document, this._host, this._container);
      case DyTrigger.HOVER:
        return new DyHoverTriggerStrategy(this._document, this._host, this._container);
      case DyTrigger.FOCUS:
        return new DyFocusTriggerStrategy(this._document, this._host, this._container);
      case DyTrigger.NOOP:
        return new DyNoopTriggerStrategy(this._document, this._host, this._container);
      default:
