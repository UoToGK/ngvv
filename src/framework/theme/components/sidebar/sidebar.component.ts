...
import { DySidebarService, getSidebarState$, getSidebarResponsiveState$ } from './sidebar.service';

export type DySidebarState = 'expanded' | 'collapsed' | 'compacted';
export type DySidebarResponsiveState = 'mobile' | 'tablet' | 'pc';
 ...
 */
@Component({
  selector: 'dy-sidebar-header',
  template: `
    <ng-content></ng-content>
  `,
})
export class DySidebarHeaderComponent {
}
 ...
 */
@Component({
  selector: 'dy-sidebar-footer',
  template: `
    <ng-content></ng-content>
  `,
})
export class DySidebarFooterComponent {
}
 ...
 * ### Installation
 *
 * Import `DySidebarModule.forRoot()` to your app module.
 * ```ts
 * @NgModule({
 *   imports: [
 *     // ...
 ...
 * ```
 * and `DySidebarModule` to your feature module where the component should be shown:
 * ```ts
 * @NgModule({
 *   imports: [
 *     // ...
...
@Component({
  ...
  template: `
    <div class="main-container"
      ...
      <ng-content select="dy-sidebar-header"></ng-content>
      <div class="scrollable" (click)="onClick($event)">
        <ng-content></ng-content>
      </div>
      <ng-content select="dy-sidebar-footer"></ng-content>
    </div>
  `,
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class DySidebarComponent implements OnInit, OnDestroy {

  protected readonly responsiveValueChange$: Subject<boolean> = new Subject<boolean>();
  protected responsiveState: DySidebarResponsiveState = 'pc';
  ...
  static ngAcceptInputType_right: DyBooleanInput;
  ...
  static ngAcceptInputType_end: DyBooleanInput;
  ...
  static ngAcceptInputType_containerFixed: DyBooleanInput;
   ...
   */
  @Input()
  get state(): DySidebarState {
    return this._state;
  }
  set state(value: DySidebarState) {
    this._state = value;
  }
  protected _state: DySidebarState = 'expanded';
   ...
   */
  @Output() readonly stateChange = new EventEmitter<DySidebarState>();

  /**
   * Emits whenever sidebar responsive state change.
   */
  @Output() readonly responsiveStateChange = new EventEmitter<DySidebarResponsiveState>();
  ...
  onClick(event): void {
    const menu = this.element.nativeElement.querySelector('dy-menu');
  ...
  protected subscribeToMediaQueryChange() {
    combineLatest([
      ...
      .subscribe(([prev, current]: [DyMediaBreakpoint, DyMediaBreakpoint]) => {
  ...
  protected getMenuLink(element: HTMLElement): HTMLElement | undefined {
    if (!element || element.tagName.toLowerCase() === 'dy-menu') {
      return;
    }
  ...
  protected updateState(state: DySidebarState): void {
    if (this.state !== state) {
      this.state = state;
  ...
  /**
   * @deprecated Use DySidebarState type instead
   * @breaking-change Remove @8.0.0
   */
  ...
  /**
   * @deprecated Use DySidebarResponsiveState type instead
   * @breaking-change Remove @8.0.0
   */
