...
import { DyButton } from './base-button';
 ...
 *
 * ```html
 * <button dyButton></button>
 * ```
 * ### Installation
 *
 * Import `DyButtonModule` to your feature module.
 * ```ts
 * @NgModule({
 *   imports: [
 *     // ...
 ...
 *
 * `dyButton` could be applied to the following selectors - `button`, `input[type="button"]`, `input[type="submit"]`
 * and `a`:
 ...
 */
@Component({
  selector: 'button[dyButton],a[dyButton],input[type="button"][dyButton],input[type="submit"][dyButton]',
  template: `
    <ng-content></ng-content>
  `,
  providers: [
    { provide: DyButton, useExisting: DyButtonComponent },
  ],
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class DyButtonComponent extends DyButton implements AfterViewInit {
  /**
   ...
   * <a dyButton [disabled]="true" (click)="clickHandler()">...</a>
