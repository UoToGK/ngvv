...
import { DyPortalDirective } from '../cdk/overlay/mapping';
import { DyPlatform } from '../cdk/platform/platform-service';
import { DyDateService, DyDayPeriod } from '../calendar-kit/services/date.service';
...
import { DY_TIME_PICKER_CONFIG, DySelectedTimePayload, DyTimePickerConfig } from './model';

interface DyTimePartOption {
  value: number,
  text: string,
}
 ...
 */
@Component({
  selector: 'dy-timepicker',
  templateUrl: './timepicker.component.html',
  styleUrls: ['./timepicker.component.scss'],
  exportAs: 'dyTimepicker',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class DyTimePickerComponent<D> implements OnChanges, OnInit {
  protected blur$: Subject<void> = new Subject<void>();

  fullTimeOptions: D[];
  hoursColumnOptions: DyTimePartOption[];
  minutesColumnOptions: DyTimePartOption[];
  secondsColumnOptions: DyTimePartOption[];
  readonly dayPeriodColumnOptions = [ DyDayPeriod.AM, DyDayPeriod.PM ];
  hostRef: ElementRef;
  isAM = true;
  ...
  set date(date: D) {
    this._date = date;
    this.isAM = this.dateService.getDayPeriod(this.date) === DyDayPeriod.AM;
   ...
   * */
  @Output() onSelectTime: EventEmitter<DySelectedTimePayload<D>> = new EventEmitter<DySelectedTimePayload<D>>();
  @ViewChild(DyPortalDirective, {static: true}) portal: DyPortalDirective;

  constructor(@Inject(DY_TIME_PICKER_CONFIG) protected config: DyTimePickerConfig,
              protected platformService: DyPlatform,
  ...
  changeDayPeriod(dayPeriodToSet: DyDayPeriod): void {
    ...
    const direction = dayPeriodToSet === DyDayPeriod.AM ? -1 : 1;
  ...
  trackByTimeValues(index, item: DyTimePartOption): number {
    return item.value;
  }
  ...
  trackByDayPeriod(index, item: DyDayPeriod): string {
    return item;
  }
  ...
  isSelectedDayPeriod(dayPeriod: DyDayPeriod): boolean {
    if (this.date) {
  ...
  protected generateHours(): DyTimePartOption[] {
  ...
  protected generateMinutesOrSeconds(): DyTimePartOption[] {
