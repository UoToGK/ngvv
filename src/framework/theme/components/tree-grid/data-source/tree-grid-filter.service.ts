 ...
 */
@Injectable()
export class DyTreeGridFilterService<T> {
  filter(query: string, data: DyTreeGridPresentationNode<T>[]): DyTreeGridPresentationNode<T>[] {
    if (!query) {
      return data;
    }

    return data.reduce((filtered: DyTreeGridPresentationNode<T>[], node: DyTreeGridPresentationNode<T>) => {
      let filteredChildren: DyTreeGridPresentationNode<T>[];
