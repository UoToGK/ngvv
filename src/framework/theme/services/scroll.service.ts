 ...
 */
export interface DyScrollPosition {
 ...
 */
@Injectable()
export class DyLayoutScrollService {

  private scrollPositionReq$ = new Subject<any>();
  private manualScroll$ = new Subject<DyScrollPosition>();
   ...
   *
   * @returns {Observable<DyScrollPosition>}
   */
  getPosition(): Observable<DyScrollPosition> {
    return Observable.create((observer: Subscriber<DyScrollPosition>) => {
      const listener = new Subject<DyScrollPosition>();
   ...
   * @returns Observable<DyScrollPosition>.
   */
  onManualScroll(): Observable<DyScrollPosition> {
    return this.manualScroll$.pipe(share<DyScrollPosition>());
  }
