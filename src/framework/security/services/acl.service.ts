...
import { DY_SECURITY_OPTIONS_TOKEN, DyAclOptions, DyAclRole, DyAccessControl } from '../security.options';
 ...
 */
@Injectable()
export class DyAclService {

  private static readonly ANY_RESOURCE = '*';

  private state: DyAccessControl = {};

  constructor(@Optional() @Inject(DY_SECURITY_OPTIONS_TOKEN) protected settings: DyAclOptions = {}) {
  ...
  /**
   * Set/Reset ACL list
   * @param {DyAccessControl} list
   */
  setAccessControl(list: DyAccessControl) {
  ...
  private getRole(role: string): DyAclRole {
    return this.state[role];
  }

  private validateRole(role: string) {
    if (!role) {
      throw new Error('DyAclService: role name cannot be empty');
    }
  }

  private validateResource(resource: string) {
    if (!resource || [DyAclService.ANY_RESOURCE].includes(resource)) {
      throw new Error(`DyAclService: cannot use empty or bulk '*' resource placeholder with 'can' method`);
    }
  }

  private exactCan(role: string, permission: string, resource: string) {
    const resources = this.getRoleResources(role, permission);
    return resources.includes(resource) || resources.includes(DyAclService.ANY_RESOURCE);
  }
